...
import makeSelectTeamsState, { makeSelectTeams } from '../selectors';

const mockTeamsState = fromJS({
  teams: {
    teams: [
      {
        id: 1,
...
describe('makeSelectTeams', () => {
  const teamsSelector = makeSelectTeams();

  it('should select teams in order', () => {
    expect(teamsSelector(mockTeamsState)[0].players[0].id).toEqual('76561197996947902');
  });
});

describe('makeSelectTeamsState', () => {
  const teamsStateSelector = makeSelectTeamsState();

  it('should select the teams state', () => {
    expect(teamsStateSelector(mockTeamsState)).toBeDefined();
  });
});
